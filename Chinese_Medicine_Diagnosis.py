...
def test(foler_file_name):
	...
	for i in range (0,len(data)):
		...
		if i%7 == 6:
			t4.append(data[i].replace('\n', ''))

	for j in range (0,len(de)):
		print(de[j])

	print('--------------------------------------')
	input_sym = input(colored('Select Your Symptom: ', 'cyan'))
	print('--------------------------------------')

	print('(1)',t1[int(input_sym)-1].split('：')[0])
	print('(2)',t2[int(input_sym)-1].split('：')[0])
	print('(3)',t3[int(input_sym)-1].split('：')[0])
	print('(4)',t4[int(input_sym)-1].split('：')[0])

	print('--------------------------------------')
	input_type = input(colored('Select Your Type: ', 'cyan'))
	print('--------------------------------------')

	if input_type == str(1):
		print(colored(t1[int(input_sym)-1].split('：')[1], 'yellow'))
	if input_type == str(2):
		print(colored(t2[int(input_sym)-1].split('：')[1], 'yellow'))
	if input_type == str(3):
		print(colored(t3[int(input_sym)-1].split('：')[1], 'yellow'))
	if input_type == str(4):
		print(colored(t4[int(input_sym)-1].split('：')[1], 'yellow'))
